...
//Peticion buscar evento, realizar la visualizacion dela categoria 
User.searchEvent = (nombre, date, categoria, lugar, result) => {
  ...
  if (categoria) {
    ...
    sql =sql + "id_categoria IN (select id_categoria FROM categoria_evento where descrip_cat LIKE ?)";
    ya = true;
...
User.searchEventByUserId = (id, nombre, date, categoria, lugar, result) => {
  ...
  if (ya){
    sql += "AND id_evento in (SELECT id_evento FROM evento_interes WHERE id_estado !=4 AND id_estado != 1 AND id_usuario = ?)";
    parametros.push(id);
  }
